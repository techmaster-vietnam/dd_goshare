...
func getRolesForRoute(path string, method string) map[int]bool {
	database := GetDB()
	if database == nil {
	...
	err := database.Raw(`
        ...
        WHERE ru.path = ? AND ru.method = ?
    `, path, method).Scan(&roleIDs).Error
	if err != nil {
		return make(map[int]bool)
	}
...
func RegisterBusinessRoute(group fiber.Router, method, path string, isPrivate bool, handler fiber.Handler) {
	...
	case "GET":
		Get(group, path, exp, isPrivate, handler)
	case "POST":
		Post(group, path, exp, isPrivate, handler)
	case "PUT":
		Put(group, path, exp, isPrivate, handler)
	case "DELETE":
		Delete(group, path, exp, isPrivate, handler)
	case "PATCH":
		Patch(group, path, exp, isPrivate, handler)
	default:
